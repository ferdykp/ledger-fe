...
async function loadShared() {
  ...
  try {
    ...
    const res = await cache.match("/__ledger_shared_file__");
    if (res) {
      ...
      await cache.delete("/__ledger_shared_file__");
...
async function scan() {
  ...
  try {
    ...
    if (e.code === "ECONNABORTED") {
      error.value =
        "OCR membutuhkan waktu terlalu lama. Coba lagi dengan gambar yang lebih jelas.";
      return;
    }
    ...
    if (e.response?.status === 422) {
      error.value = e.response?.data?.message || "Bukti belum berhasil dibaca.";
      return;
    }
  ...
  <div class="page-shell max-w-6xl">
    ...
    <div v-if="fromShare" class="notice notice-primary">
      ...
      <div>
        ...
        <p>
          Ledger sedang menyiapkan bukti yang kamu bagikan dari aplikasi lain.
        </p>
      </div>
    </div>
    <section class="grid xl:grid-cols-[1.05fr_.95fr] gap-5">
      <div class="surface-card p-5 md:p-7">
        <div class="section-heading">
          ...
          <button v-if="file" @click="reset" class="icon-button">
            <X class="w-4 h-4" />
          </button>
        </div>
        <label class="upload-zone" :class="{ 'has-file': file }">
          ...
          <div v-if="!file">
            <b>Ambil foto atau pilih bukti</b
            ><span>JPG, PNG, WEBP, PDF • maks. 10 MB</span>
          </div>
      ...
      <div class="surface-card p-5 md:p-7 h-fit">
        ...
        <div v-if="loading" class="empty-state">
          <Loader2 class="w-8 h-8 animate-spin text-primary-600" /><b
            >Sedang membaca bukti</b
          ><span>Biasanya hanya membutuhkan beberapa detik.</span>
        </div>
          ...
          <div class="review-grid">
            <div>
              ...
              ><strong
                >Rp
                {{
                  Number(result.draft?.amount || 0).toLocaleString("id-ID")
                }}</strong
              >
            </div>
            <div>
